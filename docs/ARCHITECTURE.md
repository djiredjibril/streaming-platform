...
**Fix (v2 — voir historique git pour la première tentative)** : chaque `schema.prisma` fixe `generator client { output = "../src/generated/prisma-client" }` — généré **hors de `node_modules`** plutôt que dans un `node_modules/@prisma/client` local au service. Une première tentative avait essayé ce dossier local dans `node_modules` (résolu en priorité par Node avant celui de la racine) ; ça fonctionnait pour l'isolation mais cette version de Prisma résout mal un `output` personnalisé situé dans `node_modules` (`prisma generate` échouait avec "Could not resolve @prisma/client despite the installation that we just tried"). Générer complètement en dehors de `node_modules` évite les deux problèmes. Chaque site d'import utilise un chemin relatif (`'../generated/prisma-client/index.js'`) plutôt que le spécificateur nu `@prisma/client`. Le seul endroit qui a légitimement besoin du client d'un *autre* service (le test d'intégration de la Gateway, qui construit son propre `PrismaClient` pour `buildIdentityServer`/`buildCatalogServer`) passe par un export dédié (`createPrismaClient()` dans `infra/prismaClient.ts` de chaque service) plutôt que d'importer le client généré directement.

Le dossier généré (`src/generated/prisma-client/`) est ignoré par git (`.gitignore`) comme `dist/` — code généré, jamais commité.

**Piège additionnel** : `npm install` "nettoie" ce que npm ne reconnaît pas comme géré par lui dans `node_modules` — un dossier généré manuellement à l'intérieur de `node_modules` peut disparaître au prochain `npm install` ailleurs dans le monorepo (constaté avec la v1 du fix ci-dessus). Générer hors de `node_modules` supprime ce risque, mais chaque service garde un script `postinstall: "prisma generate"` de toute façon, puisque le dossier généré n'est pas commité et doit exister après tout `npm install`/`git clone` (ne nécessite pas de connexion DB, juste le schéma).
