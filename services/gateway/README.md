...
- `Query.title(slug): Title` — `null` (pas une erreur) pour tout ce que `CatalogService.GetTitleBySlug` renvoie en `NOT_FOUND` : un slug inconnu et un titre réel non publié sont indistinguables à ce niveau aussi
- `Mutation.createTitle(input): Title!` — nécessite un token d'accès **admin** (`Account.isAdmin` via `IdentityService.ValidateToken`, vérifié dans `requireAdmin()` avant même d'appeler Catalog) ; erreurs typées (`extensions.code`: `UNAUTHENTICATED`/`FORBIDDEN`/`BAD_USER_INPUT`/`ALREADY_EXISTS`/`INTERNAL_SERVER_ERROR`), jamais l'erreur gRPC brute exposée au client (`services/AGENT.md` §8)
