...
- `Query.title(slug): Title` — `null` (pas une erreur) pour tout ce que `CatalogService.GetTitleBySlug` renvoie en `NOT_FOUND` : un slug inconnu et un titre réel non publié sont indistinguables à ce niveau aussi. Expose `isPlayable`/`videoUrl`, dérivés du statut du `MediaAsset` (`docs/03-catalog.md`'s `Episode.isPlayable`, appliqué ici au niveau `Title` tant qu'il n'y a pas d'`Episode`), et `genres` (`[String!]!`, pas le `[Genre!]!` cible de la spec — rien ne cherche un titre par id de genre pour l'instant)
- `Query.browseTitles(genre, type, cursor, limit): TitleConnection!` — **public**, comme `title(slug)`, aucune vérification admin. `TitleConnection` n'est délibérément pas une vraie Relay Connection (pas de curseur par arête, juste `titles` + un seul `nextCursor` pour toute la page) : `CatalogService.BrowseTitles` ne renvoie qu'un curseur (celui du dernier titre), la pagination n'étant que "page suivante", jamais "reprendre depuis une arête arbitraire"
