...
**Rôle admin minimal** (`docs/01-identity.md`, section "Modèle d'autorisation : RBAC") : `Account.isAdmin`, un booléen plat distinct du `RoleName` de profil (OWNER/MEMBER/KID). **Aucun endpoint ne le positionne à `true`** — c'est un choix délibéré (pas de mutation self-service pour éviter qu'un compte s'auto-promeuve) : la seule façon de promouvoir un compte est une écriture DB directe, ex. `UPDATE accounts SET is_admin = true WHERE email = '...';`. Embarqué dans le claim JWT (`admin`) à `Login`/`RefreshToken` (`domain/tokens.ts`) plutôt que relu à chaque `ValidateToken` — la promotion prend donc effet au prochain `Login`/`RefreshToken` du compte, pas immédiatement (`ValidateTokenResponse.is_admin` et `Account.is_admin` reflètent ce qui était vrai au moment de la signature).
